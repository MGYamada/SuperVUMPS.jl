...
Zygote.@adjoint function svumps(h::Array{T}, A; kwargs...) where T
    ...
    X, function (Δ)
        if all(Δ[2 : end] .=== nothing)
            if T <: Real
                (Δ[1] .* T.(real.(ein"ijk, (klm, (inp, pom)) -> jlno"(Abar.AL, Abar.AC, A.AL, A.AC))), nothing) # fix later
            else
                (Δ[1] .* T.(ein"ijk, (klm, (inp, pom)) -> jlno"(Abar.AL, Abar.AC, A.AL, A.AC)), nothing) # fix later
            end
        else
